...
class OverlapPeaks(object):
        def __init__(self, rep1_peaks, rep2_peaks, pooled_peaks, pooledpr1_peaks, pooledpr2_peaks,
                ...
                self.overlapping_peaks_bb_fn = overlapping_peaks_fn + '.bb'
                self.rejected_peaks_fn		= '%s.rejected.%s' %(basename, peak_type)
                self.rejected_peaks_bb_fn	= rejected_peaks_fn + '.bb'
        ...
        def process(self):
                ...
                self.npeaks_in 		= common.count_lines(common.uncompress(pooled_peaks_fn))
                self.npeaks_out 		= common.count_lines(overlapping_peaks_fn)
                self.npeaks_rejected = common.count_lines(rejected_peaks_fn)
        ...
        def upload(self, uploader)
                ...
                overlapping_peaks 		= uploader.upload(common.compress(self.overlapping_peaks_fn))
                overlapping_peaks_bb 	= uploader.upload(self.overlapping_peaks_bb_fn)
                rejected_peaks 			= uploader.upload(common.compress(self.rejected_peaks_fn))
                rejected_peaks_bb 		= uploader.upload(self.rejected_peaks_bb_fn)
